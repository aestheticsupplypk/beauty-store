...
function CheckoutInner({ initialReferralCode }: CheckoutClientProps) {
  ...
  const [refCode, setRefCode] = useState<string>(initialReferralCode || "");
...
export default function CheckoutClient({ initialReferralCode }: CheckoutClientProps) {
  return (
    <Suspense fallback={<div className="max-w-5xl mx-auto p-6">Loading checkout…</div>}>
      <CheckoutInner initialReferralCode={initialReferralCode} />
    </Suspense>
  );
}
